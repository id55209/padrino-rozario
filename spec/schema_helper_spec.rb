...
RSpec.describe SchemaHelper do
  ...
  describe '#photo_image_schema' do
    let(:photo) { MockPhoto.new }
    
    it 'generates schema for album photos' do
      result = helper_with_subdomain.photo_image_schema(photo)
      expect(result).to include('"@type":"ImageObject"')
      expect(result).to include('"name":"Test Photo"')
      expect(result).to include('"description":"Test Photo"')
      expect(result).to include('"author":"Rozario Flowers"')
    end
  end
